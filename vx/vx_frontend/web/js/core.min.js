"use strict";class CustomerData{constructor(e){this.baseUrl=e,this.storageKey="vira-mage-cache-storage"}async init(){try{let e=await fetch(`${this.baseUrl}customer/section/load`);if(!e.ok)throw Error("Network response was not ok");let t=await e.json();this.saveToLocalStorage(t)}catch(r){console.error("Error while initializing the customer data:",r.message)}}saveToLocalStorage(e){localStorage.setItem(this.storageKey,JSON.stringify(e))}loadFromLocalStorage(){let e=localStorage.getItem(this.storageKey);return e?JSON.parse(e):null}get(e){let t=this.loadFromLocalStorage();return t&&t[e]?t[e]:null}}const customerData=new CustomerData(BASE_URL);async function getCustomerDataSections(e){let t=`${BASE_URL}customer/section/load/?sections=${e}`;try{let r=await fetch(t,{method:"GET",headers:{"Content-Type":"application/json","X-Requested-With":"XMLHttpRequest"}});if(!r.ok)throw Error(`Network response was not ok: ${r.statusText}`);return await r.json()}catch(a){console.error("Error fetching customer section:",a)}}function getCookieValue(e){let t=document.cookie.match(RegExp("(^|;\\s*)"+e+"=([^;]*)"));return t?decodeURIComponent(t[2].replace(/\+/g," ")):null}function displayMessages(e){let t=document.getElementById("message-manager-se");t&&(e.forEach(e=>{let r=document.createElement("div");r.className=`p-3 ${getMessageClass(e.type)} rounded border font-montmedium flex justify-start items-center w-full mb-2`,r.innerHTML=`
      <div>${e.text}</div>
...
new PerformanceObserver((list) => {
    for (const entry of list.getEntries()) {
        ...
        console.trace(); // This will print the stack trace to help identify where the long task originates
    }
